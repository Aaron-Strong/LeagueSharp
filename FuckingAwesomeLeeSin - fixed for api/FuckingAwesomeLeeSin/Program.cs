...
namespace FuckingAwesomeLeeSin
{
    internal static class Program
    {
        ...
        public static void SaveMe()
        {
                    ...
                    Player.Position, 1100f, MinionTypes.All, MinionTeam.Neutral, MinionOrderTypes.None))
            {
                if (
                    epics.Any(
                        minionName =>
                            minion.CharData.BaseSkinName == minionName && HpLowerParam(minion, "hpEpics") && ParamBool("dEpics")))
                {
                    epicSafe = true;
                }
                if (
                    buffs.Any(
                        minionName =>
                            minion.CharData.BaseSkinName == minionName && HpLowerParam(minion, "hpBuffs") && ParamBool("dBuffs")))
                {
                    buffSafe = true;
                }
            }
        ...
        public static void smiter()
        {
            var minion =
                ObjectManager.Get<Obj_AI_Minion>()
                    .FirstOrDefault(a => buffandepics.Contains(a.CharData.BaseSkinName) && a.Distance(Player) <= 1300);
            if (minion != null)
            {
                if (Menu.Item(minion.CharData.BaseSkinName).GetValue<bool>())
                {
                    minionerimo = minion;
